package com.davidshibru.taskflow.features.profile.presentation
...
fun ScreenScope.profileScreen() {
    toolbar = ScreenToolbar.Default(title = "Profile")
    
    content {
        val viewModel = viewModel<ProfileViewModel>()
        val container: Container<ProfileViewModel.State> by viewModel.stateFlow.collectAsState()
...
@ScreenPreview
@Composable
private fun ProfileContentPreview() = PreviewScreenContent {
    ProfileContent(
        state = ProfileViewModel.State(
            title = "Profile",
            counter = 3,
